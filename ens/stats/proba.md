---
title: Outils et rappels de probabilités
order: 1
---

# Outils et rappels de probabilités

## Convergence de variables aléatoires

*Dans cette partie, nous allons définir trois manières que peut avoir une suite de vraiables aléatoires de converger. On considère donc $(X_n)_{n \in \mathbb{N}}$ une suite de variables aléatoires réelles et $X$ une variable aléatoire réelle, sa potentielle limite.*

> [!DÉFINITION]
>
> On dit que $(X_n)_{n \in \mathbb{N}}$ converge en probabilité vers $X$ lorsque
>
> $$\forall \epsilon > 0, lim_{n \rightarrow \infty } \mathbb{P} (|X_n - X| > \epsilon) = 0$$
>
> On note alors
>
> $$X_n \longrightarrow^{\mathbb{P}}_{n\rightarrow \infty} X$$

**Exemple**: soit $(X_n)_{n \in \mathbb{N}}$ une suite de variables aléatoires telle quel $\forall n \in \mathbb{N}^*$, $X_n \sim \mathcal{B}(\frac pn)$ avec $p \in ]0, 1[$. Montrons que $(X_n)_{n \in \mathbb{N}}$ converge vers $0$ en probabilités.

💡 *La propriété doit être vraie pour tout $\epsilon$. On prend donc un epsilon quelconque*

Soit $\epsilon > 0$ alors $\mathbb{P}(|X_n - 0| > \epsilon) = \mathbb{P} (X_n > \epsilon) = 0$ si $

> [!définition] Convergence presque sûr
> On dit que $(X_n)_{n \in \mathbb{N}}$ **converge presque surement** vers $l$ lorsque $\mathbb{P} (lim X_n = l) = 1$ et on note $X_n \longrightarrow^{\mathbb{P.S.}} X$

*Pour démontrer qu'une suite de variables aléatoires converge presque surement, on peut utiliser le résultat suivant :*

> [!définition] Lemme de <span style="font-family: Playfair Display">Borel Cartelli</span>
> Si $\forall \epsilon > 0, \sum_{n = 1}^{+ \infty}  \mathbb{P} (|X_n - l| > \epsilon) < \infty$, alors $X_n$ *converge presque surement vers* $l$.

## Inégalités Classiques

> [!définition] Inégalité de Markov
>
> Soient $p$ et $c$ deux constantes positives et $\mathcal{X}$ une v.a.r. tel que $\mathbb{E} (|\mathcal{X}|) < \infty$ ($\mathcal{X}$ possède une masse au centre). Alors :
>
> $$\mathbb{P} (|\mathcal{X}| > c ) \le \frac{\mathbb{E}(|\mathcal{X}|)}{c}$$

*On obtient l*

> [!définition] Inégalité de Bienaymé Chevychev

> [!définition] Inégalité de Jensen

> [!définition] Inégalité de Cauchy-Schwarz

## Résultats Asymptotiques

### Théorèmes Limites

> [!définition] Loi faible des grand nombres

> [!définition] Loi forte des grand nombres

> [!définition] Théorème Central Limite

### Opérations sur les limites

> [!définition] Théorème de continuité

> [!définition] Théorème Slutsky

## Variables Aléatoires à densités

> [!définition] Définition : Variable aléatoire à densité
