...
Malgré le symbole de somme, les séries ne sont pas des *sommes* mais des *suites* !

On va chercher les natures des séries que nous utilisons, car une certaine nature peut donner accès à plus de propriétés. Étudier la nature d'une suite correspond à trouver sa limite, et on la calcule de la manière suivante :
...
Si la limite existe **et uniquement si elle existe**, on peut alors écrire $\sum_{k_0}^{+\infty} u_k$. *On a donc $\sum_{k_0}^{+\infty} u_k = lim_{n \rightarrow +\infty} S_n$.* On calculera donc les sommes partielles avant les limites complètes pour s'assurer de ne pas écrire de contre sens.
...
- On ne change pas la nature d'une série (convergente ou divergente) en supprimant un nombre fini de termes. $\sum_{n \gt n_1>n_0} \sim \sum_{n \gt n_0}$ 
- Si une série de terme général $u_n$ converge, alors $\lim_{x\to\infty} u_n = 0$ (mais pas de réciproques).
- *La contraposée est aussi valable,* si la limite ne vaut pas 0 alors la suite est divergente (mais pas de réciproques).
- Les séries sont linéaires à condition d'être réelles (donc convergentes). De plus, si la série $\sum v_n = \sum \lambda u_n$, alors $\sum v_n = \lambda \sum u_n$ et la série de $v_n$ a la même nature que celle de $u_n$.
...
- On peut également comparer les séries et les sommes de séries
- Si $u_n \sim v_n$ alors leurs séries sont de même nature.
- Si $u_n = o(v_n)$ *et $v_n$ abolument convergente* alors leurs séries sont de même nature.
- Si $u_n$ est une suite réelle décroissante convergant vers 0, alors la série $\sum (-1)^n u_n$ est convergente (**suites alternées**).

> [!warning] Il est **NÉCESSAIRE** que le/les suites soient à termes positifs !
...
$$\sum_{\color{red}n = 1}^{+\infty} nq^{n - 1} = \frac{\color{red}1}{(1 -q)^{\color{red}2}}$$
...
$$\sum_{\color{cyan}n =2}^{+\infty} n(n-1)q^{n -2} = \frac{\color{cyan}2}{(1 -q)^{\color{cyan}3}}$$
...
Par définition, la série est **toujours convergente** et on a
